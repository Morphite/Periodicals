...
<div class="container">
    <h3 class="mt-4 text-center"><fmt:message key="title.admin.create-theme" bundle="${bundle}"/></h3>

    <form action="period" method="post" accept-charset="UTF-8">
        <input type="hidden" name="command" value="create_theme">

        <div class="form-row">
            <div class="form-group col-md-6">
                <label for="inputName"><fmt:message key="label.name" bundle="${bundle}"/></label>
                <input type="text" pattern="([A-ZА-Яa-zа-я0-9_- ]){3,45}" name="name" class="form-control" id="inputName"
        ...
        <div class="form-row">
            <div class="form-group col-md-6">
                ...
                <input type="text" pattern="([A-ZА-Яa-zа-я0-9_- ]){5,45}" name="imgPath" class="form-control" id="inputImgPath" placeholder=<fmt:message
                        key="label.imgPath" bundle="${bundle}"/> required>
            </div>
        </div>
        ...
        <c:choose>
            <c:when test="${ not empty errorThemeCreate }">
                <br/>
                <span style="color: #ff0000;">${errorThemeCreate}</span>
                <br/>
            </c:when>
            <c:otherwise>
            </c:otherwise>
